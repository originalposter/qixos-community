  ...
  solutions = {
    ...
    backend = {
      ...
      execer = [
        ...
        # gnupg itself, so it is covered. pinentry is not, and is left out on
        # purpose rather than by oversight: it is the agent that runs it, not
        # gpg2, and the agent takes its pinentry-program from
        # /etc/gnupg/gpg-agent.conf. That path belongs to the backend qube's
        # config, so the server module's `pinentry` option supplies it and
        # keeps it in the system closure instead. Passphrase-protected backend
        # keys do work; see that option for why.
